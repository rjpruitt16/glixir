defmodule Glixir.Registry do
  ...
  def start_registry(name, keys_type) when is_binary(name) and is_binary(keys_type) do
    Logger.debug("[Registry.ex] Starting Registry: #{name} with keys: #{keys_type}")

    registry_name = String.to_atom(name)
    keys_atom = String.to_atom(keys_type)

    Logger.debug(
      "[Registry.ex] Converted to atoms: registry_name=#{inspect(registry_name)}, keys=#{inspect(keys_atom)}"
    )

    case Registry.start_link(keys: keys_atom, name: registry_name) do
      {:ok, pid} ->
        Logger.info("[Registry.ex] ✅ Registry '#{name}' started successfully: #{inspect(pid)}")
        {:registry_start_ok, pid}

      {:error, {:already_started, pid}} ->
        Logger.debug("[Registry.ex] ⚠️ Registry '#{name}' already started: #{inspect(pid)}")
        {:registry_start_ok, pid}

      {:error, reason} ->
        Logger.error("[Registry.ex] ❌ Registry '#{name}' start failed: #{inspect(reason)}")
  ...
  def register_subject(registry_name, key, subject)
    ...
    Logger.debug("[Registry.ex] Registering subject in '#{registry_name}' with key: #{key}")
    Logger.debug("[Registry.ex] Subject received: #{inspect(subject)}")

    registry_atom = String.to_atom(registry_name)
    Logger.debug("[Registry.ex] Registry atom: #{inspect(registry_atom)}")
    ...
    {pid, metadata} =
      case subject do
        {:subject, pid, ref} ->
          Logger.debug(
            "[Registry.ex] ✅ Extracted PID #{inspect(pid)} and ref #{inspect(ref)} from Gleam Subject"
          )

          {pid, %{gleam_subject: subject, ref: ref}}

        pid when is_pid(pid) ->
          Logger.debug("[Registry.ex] ⚠️ Got raw PID: #{inspect(pid)}")
          {pid, %{gleam_subject: subject}}

        other ->
          Logger.warning("[Registry.ex] ⚠️ Unexpected subject format: #{inspect(other)}")
          {other, %{gleam_subject: subject}}
      end

    Logger.debug(
      "[Registry.ex] About to register PID #{inspect(pid)} with key '#{key}' and metadata: #{inspect(metadata)}"
    )

    case Registry.register(registry_atom, key, metadata) do
      {:ok, _owner} ->
        Logger.info(
          "[Registry.ex] ✅ Subject registered successfully in '#{registry_name}' with key: #{key}"
        )
      ...
      {:error, {:already_registered, existing_pid}} ->
        Logger.warning(
          "[Registry.ex] ⚠️ Key '#{key}' already registered to #{inspect(existing_pid)}"
        )

        {:registry_register_error, {:already_registered, existing_pid}}

      {:error, reason} ->
        Logger.error("[Registry.ex] ❌ Registration failed for key '#{key}': #{inspect(reason)}")
  ...
  def lookup_subject(registry_name, key) when is_binary(registry_name) and is_binary(key) do
    Logger.debug("Looking up subject in '#{registry_name}' with key: #{key}")
    ...
    try do
      ...
      case Registry.lookup(registry_atom, key) do
        [{_pid, metadata}] ->
          ...
          Logger.debug("Found subject for key '#{key}': #{inspect(subject)}")
          {:registry_lookup_ok, subject}

        [] ->
          Logger.debug("No subject found for key '#{key}' in registry '#{registry_name}'")
        ...
        multiple when is_list(multiple) and length(multiple) > 1 ->
          ...
          Logger.warning("Multiple entries found for key '#{key}': #{inspect(multiple)}")
      ...
      ArgumentError ->
        ...
        Logger.debug("Registry '#{registry_name}' does not exist")
        {:registry_lookup_error, :registry_not_found}

      error ->
        Logger.error("Exception during lookup for key '#{key}': #{inspect(error)}")
  ...
  def unregister_subject(registry_name, key) when is_binary(registry_name) and is_binary(key) do
    Logger.debug("Unregistering key '#{key}' from registry '#{registry_name}'")
    ...
    try do
      ...
      case Registry.unregister(registry_atom, key) do
        :ok ->
          Logger.info("Successfully unregistered key '#{key}' from registry '#{registry_name}'")
          :registry_unregister_ok

        {:error, reason} ->
          Logger.error("Unregistration failed for key '#{key}': #{inspect(reason)}")
          {:registry_unregister_error, reason}
      end
    rescue
      error ->
        Logger.error("Exception during unregistration for key '#{key}': #{inspect(error)}")
